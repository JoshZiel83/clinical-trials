...
DEFAULT_MAPPING_PATH = REFERENCE_DATA_DIR / "therapeutic_area_mapping.json"


def load_ta_mapping(json_path=None):
    """Load therapeutic area mapping from JSON file.

    Returns a list of dicts with keys: mesh_ancestor, therapeutic_area.
    """
    path = json_path or DEFAULT_MAPPING_PATH
    with open(path) as f:
        mapping = json.load(f)
...
def create_study_therapeutic_areas(duck_conn):
    ...
    duck_conn.execute("""
        ...
        SELECT DISTINCT
            bc.nct_id,
            bc.mesh_term   AS matched_ancestor,
            ta.therapeutic_area,
