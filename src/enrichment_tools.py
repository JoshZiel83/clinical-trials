...
from src.normalize_drugs import CHEMBL_SYNONYMS_PATH, normalize_drug_name
from src.normalize_sponsors import normalize_sponsor_name
...
class ToolContext:
    ...
    def chembl_targets(self) -> list[tuple[str, str, Optional[str]]]:
        if self._chembl_targets is None:
            if CHEMBL_SYNONYMS_PATH.exists():
                df = pd.read_parquet(CHEMBL_SYNONYMS_PATH).dropna(
    ...
    def sponsor_canonicals(self) -> list[str]:
        if self._sponsor_canonicals is None:
            rows = self.duck_conn.execute("""
                SELECT DISTINCT canonical_name FROM ref.sponsor_dictionary
                WHERE mapping_method IN ('exact-after-normalize', 'manual')
            """).fetchall()
...
def quickumls_condition(ctx: ToolContext, text: str, limit: int = 5) -> list[dict]:
    ...
    """
    from src.quickumls_tool import lookup
    results = lookup(text)
    return [
        {
            "cui": r["cui"],
