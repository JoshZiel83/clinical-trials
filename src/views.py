...
def build_study_summary(duck_conn):
    ...
    duck_conn.execute(f"""
        ...
        conditions_agg AS (
            SELECT
                nct_id,
                LIST(DISTINCT condition_name) AS raw_condition_names,
                LIST(DISTINCT canonical_term) FILTER (WHERE canonical_term IS NOT NULL)
                    AS canonical_conditions,
                COUNT(*) AS condition_count,
                COUNT(DISTINCT canonical_term) FILTER (WHERE canonical_term IS NOT NULL)
        ...
        drugs_agg AS (
            SELECT
                nct_id,
                LIST(DISTINCT canonical_name) FILTER (WHERE canonical_name IS NOT NULL)
                    AS canonical_drugs,
                LIST(DISTINCT canonical_id) FILTER (WHERE canonical_id IS NOT NULL)
                    AS chembl_ids,
                COUNT(*) AS drug_intervention_count,
                COUNT(DISTINCT canonical_name) FILTER (WHERE canonical_name IS NOT NULL)
        ...
        lead_sponsor AS (
            SELECT nct_id, canonical_name AS lead_sponsor_name,
                   agency_class AS lead_sponsor_agency_class
            FROM (
                SELECT
                    nct_id, canonical_name, agency_class,
                    ROW_NUMBER() OVER (PARTITION BY nct_id ORDER BY nct_id) AS rn
                FROM norm.study_sponsors
        ...
        collaborators AS (
            SELECT
                nct_id,
                LIST(DISTINCT canonical_name) AS collaborator_names
            FROM norm.study_sponsors
