...
def build_sponsor_dictionary(duck_conn):
    ...
    in each group (tiebreak: alphabetical). Each distinct raw name is
    written as one dict entry. Manual entries are preserved across
    rebuilds (same pattern as condition/drug dictionaries).
    """
    ...
    # Wipe only automated layers; preserve manual entries.
    duck_conn.execute(
    ...
    # Pick a canonical form per normalized group: highest freq, tiebreak alpha
    ...
    # Skip raw names already present as manual entries
    ...
    try:
        duck_conn.execute("""
            INSERT INTO ref.sponsor_dictionary
                (source_name, canonical_name, canonical_id,
    ...
    distinct_canonical = duck_conn.execute(
        "SELECT COUNT(DISTINCT canonical_name) FROM ref.sponsor_dictionary"
    ).fetchone()[0]
    logger.info(
        f"ref.sponsor_dictionary: {total:,} rows, "
        f"{distinct_canonical:,} distinct canonical names"
    )
    return total


def create_study_sponsors(duck_conn):
    """Create norm.study_sponsors by joining raw.sponsors to the dictionary."""
    ...
    duck_conn.execute("""
        ...
        SELECT
            ...
            COALESCE(d.canonical_name, s.name) AS canonical_name,
            s.agency_class,
    ...
    mapped = duck_conn.execute("""
        ...
        WHERE canonical_name IS NOT NULL
          AND canonical_name <> original_name
    """).fetchone()[0]
    logger.info(
        f"norm.study_sponsors: {row_count:,} rows, "
        f"{mapped:,} changed by normalization"
    )
    return row_count
...
def generate_sponsor_fuzzy_candidates(duck_conn, score_cutoff=88, top_n=2000):
    ...
    canonicals = duck_conn.execute("""
        SELECT d.canonical_name AS canonical,
