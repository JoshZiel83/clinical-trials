...
from src.transform.normalize_conditions import (
...
def test_build_dictionary_one_to_one():
    ...
    one_to_one = conn.execute("""
        ...
        FROM ref.condition_dictionary d
        JOIN entities.condition e ON d.condition_id = e.condition_id
        WHERE d.mapping_method = '1:1-study'
    """).fetchall()
...
def test_build_dictionary_co_occurrence():
    ...
    co_occur = conn.execute("""
        ...
        FROM ref.condition_dictionary d
        JOIN entities.condition e ON d.condition_id = e.condition_id
        WHERE d.mapping_method = 'co-occurrence'
    """).fetchall()
...
def test_build_dictionary_preserves_manual():
    ...
    health_id = entities.upsert_condition(
        conn, canonical_term="Health", origin="manual",
    )
    conn.execute(
        "INSERT INTO ref.condition_dictionary VALUES (?, ?, ?, ?)",
        ["healthy volunteers", health_id, "manual", "high"],
    )

    build_condition_dictionary(conn)

    manual = conn.execute("""
        SELECT condition_name, mapping_method FROM ref.condition_dictionary
...
def test_unmapped_conditions_have_null_condition_id():
    """Conditions with no dictionary entry should have NULL condition_id."""
    conn = _setup_test_db()
    build_condition_dictionary(conn)
    create_study_conditions(conn)
...
def test_manual_dictionary_entry_picked_up():
    ...
    create_study_conditions(conn)
    before = conn.execute("""
        SELECT condition_id FROM norm.study_conditions
    ...
    create_study_conditions(conn)
    row = conn.execute("""
        SELECT s.condition_id, s.mapping_method, e.canonical_term
        FROM norm.study_conditions s
        LEFT JOIN entities.condition e ON s.condition_id = e.condition_id
        WHERE s.condition_name = 'Healthy Volunteers'
    """).fetchone()
...
def test_cancer_synonym_with_qualifier():
    ...
    result = conn.execute("""
        ...
        FROM ref.condition_dictionary d
        JOIN entities.condition e ON d.condition_id = e.condition_id
        WHERE d.condition_name = 'advanced lung cancer'
    """).fetchone()
    assert result is not None
