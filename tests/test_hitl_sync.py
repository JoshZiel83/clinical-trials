...
def _fresh_conn():
    ...
    conn.execute("""
        CREATE TABLE ref.condition_dictionary (
            condition_name VARCHAR, canonical_term VARCHAR,
            mapping_method VARCHAR, confidence VARCHAR
        )
    """)
    conn.execute("""
        CREATE TABLE ref.drug_dictionary (
            source_name VARCHAR, canonical_name VARCHAR, canonical_id VARCHAR,
            mapping_method VARCHAR, confidence VARCHAR
        )
    """)
    conn.execute("""
...
def test_apply_promotes_and_marks(tmp_path, monkeypatch):
    ...
    assert total == {"approved": 1, "rejected": 1, "promoted": 1}
    assert domains == {"condition"}

    # Dictionary entry appeared
    row = conn.execute(
        "SELECT canonical_term, mapping_method FROM ref.condition_dictionary "
        "WHERE condition_name = 'bc'"
    ).fetchone()
