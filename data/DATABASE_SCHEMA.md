...
| `ref` | Reference/lookup tables for normalization. Hand-curated or derived. | 2 tables |
| `norm` | Normalized entities with provenance tracking. | 2 tables |
| `meta` | Pipeline metadata (extraction logs, run statistics). | 1 table |

Future schemas (not yet created): `class` (classification), `views` (analytical views).
...
- `manual` — hand-curated (future; preserved across automated rebuilds)
- `quickumls` — QuickUMLS mapping (future Phase 2C)

**14,222 rows** (3,247 exact + 8,263 1:1-study + 2,712 co-occurrence)
